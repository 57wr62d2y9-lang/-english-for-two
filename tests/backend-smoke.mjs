...
await writeFile('/tmp/eft-v04-qa-account-ids.json',JSON.stringify(accounts.map(a=>a.id)),{mode:0o600});
...
assert.equal((await call(0,'save_state',{records:[{key:'progress:A2:ielts-A2-café-0',at,data:{s:'LEARNING',c:1,w:1,l:at}},{key:'settings',at,data:{level:'B1',minutes:15,updatedAt:at}}]})).saved,2);
...
assert.equal(own.records.find(r=>r.record_key==='settings').payload.minutes,15);
assert.equal(own.records.find(r=>r.record_key==='progress:A2:ielts-A2-café-0').payload.w,1);
assert.equal((await call(0,'save_state',{records:[{key:'progress:B1:../../secret',at,data:{}}]})).status,400);
