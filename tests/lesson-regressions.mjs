...
import { makeSession, nextLessonTask, applySessionEvidence, isAnswerCorrect } from '../src/lesson-engine.js';
...
function simulate(level,n=65) {
  ...
  for(let i=0;i<n;i++){
    session=nextLessonTask(session,progress,time+i*10000);assert.ok(!session.exhausted,`${level} exhausted at ${i}`);
...
test('every level has a distinct programme including daily IELTS and multi-question media',()=>{
  const all={};
  for(const level of ['A2','B1','B2','C1']){
    ...
    assert.ok(tasks.some(task=>task.type==='ielts'&&task.skill==='Writing'));
    assert.ok(tasks.some(task=>task.type==='ielts'&&task.skill==='Speaking'));
    assert.ok(tasks.some(task=>task.type==='grammar'));
