...
export function MediaPrompt({task,ready,onOpen,onReady,onSkip}) {
  ...
  return <section className="mediaBlock"><h1>{lesson.title}</h1><p>{lesson.note}</p>
...
export default function SpeakingPractice({task,onComplete,completed,mode,onModeChange,draft,onDraftChange,onVideo}) {
  ...
  return <section className="speakingPractice">
    ...
    <div className="rehearsalPlan"><strong>План ответа</strong><p>{task.structureRu || task.ru}</p><p lang="en">{task.structure}</p></div>
    ...
    {task.model&&<details><summary>Пример ответа и перевод</summary><p lang="en">{task.model}</p><TranslateButton text={task.model} ru={task.modelRu || ''} label="Перевести пример"/><button type="button" className="secondary" onClick={listen}>Слушать пример · голос устройства</button></details>}
