...
import { makeSession, nextLessonTask, isAnswerCorrect, applySessionEvidence, replaceSpeakingWithVideo } from './lesson-engine.js';
import { DAILY_GRAMMAR } from './lesson-notes.js';
...
import { RuleHelp, Example, TaskTranslation, TaskHint, AnswerExplanation } from './ExerciseHelp.jsx';
import ReminderSettings from './ReminderSettings.jsx';
import SpeakingPractice, {SpeakingModeChoice, MediaPrompt} from './SpeakingPractice.jsx';
import {normaliseSpeakingMode, mediaBlockHasNext, skipUnheardMedia, resumeSpeakingMode} from './quiet-speaking.js';
import {WordHelpProvider,EnglishText,LookupHint,AnswerChoices} from './WordLookup.jsx';

const defaults={level:'B1',minutes:15,dailyGoal:30,profileName:'Artur',speakingMode:'quiet'};
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      if(loaded.draft?.data?.version===3 && !loaded.stats.lessons?.[loaded.draft.data.id] && !loaded.draft.data.done){const resumed=resumeSpeakingMode(loaded.draft.data,config);setCurrentSession(resumed);saveDraft(resumed);}
      else saveDraft(null);
  ...
  function changeSpeakingMode(mode) {
    const value=normaliseSpeakingMode(mode);changeSettings({speakingMode:value});
    if(sessionRef.current&&!sessionRef.current.done){const next={...sessionRef.current,speakingMode:value};setCurrentSession(next);saveDraft(next);}
  }
  function updateSpeakingDraft(draft) {
    const current=sessionRef.current;if(!current?.task.practice||current.feedback)return;
    const next={...current,speakingDraft:draft};setCurrentSession(next);saveDraft(next);
  }
  function switchToVideo() {
    const current=sessionRef.current;if(!current?.task.practice||current.feedback)return false;
    const next=replaceSpeakingWithVideo(current,stateRef.current.progress);if(!next)return false;
    changeSettings({speakingMode:'quiet'});setCurrentSession(next);saveDraft(next);return true;
  }
  function skipMedia() {
    if(!sessionRef.current?.mediaBlock||sessionRef.current.feedback)return;
    const next=skipUnheardMedia({...sessionRef.current,speakingMode:'quiet'});
    changeSettings({speakingMode:'quiet'});setCurrentSession(next);saveDraft(next);
  }
  function startLesson(minutes=settings.minutes) {
    ...
    const next=nextLessonTask(makeSession(s.settings.level,minutes,index,Date.now(),s.settings.speakingMode),s.progress);
  ...
  function advance(base=sessionRef.current) {
    const blockPending=mediaBlockHasNext(base.mediaBlock);
  ...
  return <div className="app v5App">
    ...
    {screen==='home'&&<>
      ...
      <section className="todayCard"><div className="todayTop"><div><div className="eyebrow">{slot==='morning'?'УТРЕННИЙ УРОК':'ВЕЧЕРНИЙ УРОК'}</div><h1>Новый шаг сегодня</h1><p>{minutes} из {settings.dailyGoal} минут за день</p></div><div className="dayRing" style={{'--done':`${Math.min(100,minutes/settings.dailyGoal*100)}%`}}><span>{Math.min(100,Math.round(minutes/settings.dailyGoal*100))}%</span></div></div>
        <SpeakingModeChoice mode={settings.speakingMode} onChange={changeSpeakingMode}/><div className="lessonChips"><span>Фразы и грамматика</span><span>Аудио / видео</span><span>IELTS · {settings.level==='A2'?'основы':settings.level}</span></div>
    ...
    {screen==='settings'&&<><ReminderSettings/><section className="settingsCard"><h2>Речевая практика</h2><SpeakingModeChoice mode={settings.speakingMode} onChange={changeSpeakingMode}/></section></>}
...
function RewardRules(){return <details className="rewardRules"><summary>Как заработать $1, $2 или $3</summary><p><b>$1</b> — 5 минут практики и 3 проверяемых ответа. В коротком уроке достаточно 4 минут. Даже с ошибками: главное — заниматься.</p><p><b>$2</b> — 12 минут обычного урока, 8 ответов, хотя бы половина верных и 2 типа заданий. Вместо точности можно исправить 2 ошибки в повторах.</p><p><b>$3</b> — 12 минут, 12 ответов, точность от 75%, 3 типа заданий и 8 разных заданий решены верно. Торопиться не нужно.</p><p>Переводы и подсказки разрешены для любой награды. За короткий урок — максимум $1. Устный или мысленный ответ — самопроверка. Баллы и точность учитывают проверяемые задания, включая вопросы по видео.</p><p>Награда — один раз утром и один раз вечером. $0 остаётся только для слишком короткой практики без 3 ответов или если награда за эту часть дня уже получена.</p></details>;}

export function Lesson({session,speakingMode,onSpeakingMode,onSpeakingDraft,onVideoAlternative,onSkipMedia,ascent,onIntro,onAnswer,onNext,onFinish,onBack,onHome,onHint,onOpenMedia,onReadyMedia,onMore}) {
  ...
  return <WordHelpProvider key={`${session.id}:${session.step}`} level={session.level} onHint={onHint}><div className="app sessionApp"><div className="topbar"><button className="back" onClick={onBack} aria-label="Сохранить урок и выйти">←</button><strong>{session.level} · задание {session.step}</strong><span className="timer">{Math.floor(seconds/60)}:{String(seconds%60).padStart(2,'0')}</span></div><div className="sessionProgress"><i style={{width:`${100-session.remainingMs/session.plannedMs*100}%`}}/></div>{seconds===0&&<p className="notice">Время вышло. Закончи {media?'блок вопросов':'это задание'}, и подведём итог.</p>}<main className="exercise" key={`${session.id}:${session.step}`}>
    ...
    {task.type==='intro'?<><div className="eyebrow">НОВОЕ ВЫРАЖЕНИЕ</div><h1 className="phrase"><EnglishText text={task.item.phrase} ru={task.item.ru}/></h1><div className="mainTranslation">{task.item.ru}</div><p className="meaning"><EnglishText text={task.item.explanation}/></p><Pronounce text={task.item.phrase}/><RuleHelp guide={task.guide} open/><div className="examples">{task.examples.slice(0,3).map(example=><Example key={example.en} example={example}/>)}</div>{task.examples.length>3&&<details className="variationBox"><summary>Ещё {task.examples.length-3} {ruPlural(task.examples.length-3,'пример','примера','примеров')}</summary>{task.examples.slice(3).map(example=><Example key={example.en} example={example}/>)}</details>}<div className="actions twoActions"><button className="primary" onClick={()=>onIntro(false)}>Потренировать</button><button className="secondary" onClick={()=>onIntro(true)}>Уже хорошо знаю</button></div><small className="helper">Знакомую фразу проверим через 35 дней. Текущий урок продолжится с другим материалом.</small></>:<>
      ...
      {media&&<MediaPrompt task={task} ready={showQuestion} onOpen={onOpenMedia} onReady={onReadyMedia} onSkip={onSkipMedia}/> }
      {showQuestion&&<>
        ...
        <h1 className="contextTitle"><EnglishText text={task.prompt} ru={task.type==='recognition'?task.item?.ru:''}/></h1>{task.subPrompt&&<p className="meaning"><EnglishText text={task.subPrompt}/></p>}<p className="prompt">{task.instruction || (task.practice?task.ru:task.type==='grammar'?'Выбери правильную форму.':'Выбери ответ.')}</p>
        {task.practice?<SpeakingPractice task={task} mode={speakingMode} onModeChange={onSpeakingMode} draft={session.speakingDraft} onDraftChange={onSpeakingDraft} onVideo={onVideoAlternative} completed={Boolean(session.feedback)} onComplete={()=>onAnswer(null)}/>:task.type==='recall'?<TextAnswer disabled={Boolean(session.feedback)} onSubmit={onAnswer}/>:task.type==='order'?<OrderAnswer task={task} disabled={Boolean(session.feedback)} onSubmit={onAnswer}/>:<AnswerChoices options={task.options} selected={session.selected} answer={task.answer} feedback={session.feedback} disabled={Boolean(session.feedback)} onAnswer={onAnswer}/>}
        {!session.feedback&&<>{!task.passage&&<TaskTranslation task={task} onHint={onHint}/>}<TaskHint task={task} onHint={onHint}/></>}
        {session.feedback&&<section className={`feedback ${session.feedback}`} aria-live="polite"><strong>{session.feedback==='practice'?(speakingMode==='quiet'?'Ответ подготовлен про себя':'Речевая практика выполнена'):session.feedback==='correct'?'Верно!':'Разберём ответ'}</strong><AnswerExplanation task={task} selected={session.selected} wrong={session.feedback==='wrong'}/>{session.feedback==='wrong'&&<p className="recoveryNotice">Ошибка сохранена. Вернёмся к ней после нескольких других заданий или в следующем занятии, когда подойдёт время повтора.</p>}<button className="primary big" onClick={onNext}>{media&&task.questionIndex+1<task.questionCount?'Следующий вопрос':media&&session.mediaBlock?.quiet?(speakingMode==='quiet'?'Пересказать про себя':'Пересказать вслух'):seconds===0?'Завершить урок':'Дальше'}</button></section>}
      </>}
    </>}
