...
import {SpeakingModeChoice} from './SpeakingPractice.jsx';
...
export default function App() {
  ...
  function answer(value) {
    ...
    const action=task.practice?'intro':!ok?'wrong':task.type==='recognition'?'recognition':task.type==='recall'?'recall':'context';
    const result=reviewItem(progressFor(task.item,stateRef.current.progress),action,Date.now(),`${current.id}:${current.step}`);
  ...
  function openCheck(quarter=0) {
    const tasks=quarter?buildCheckpoint(items,progress,quarter):buildFinalCheck(items,progress);
  ...
  return <div className="app v5App">
    ...
    {screen==='home'&&<>
      ...
      <section className="todayCard"><div className="todayTop"><div><div className="eyebrow">{slot==='morning'?'УТРЕННИЙ УРОК':'ВЕЧЕРНИЙ УРОК'}</div><h1>Новый шаг сегодня</h1><p>{minutes} из {settings.dailyGoal} минут за день</p></div><div className="dayRing" style={{'--done':`${Math.min(100,minutes/settings.dailyGoal*100)}%`}}><span>{Math.min(100,Math.round(minutes/settings.dailyGoal*100))}%</span></div></div>
        <SpeakingModeChoice mode={settings.speakingMode} onChange={changeSpeakingMode}/><div className="lessonChips"><span>Новые слова и фразы</span><span>Жизненные примеры</span><span>Повторы по памяти</span></div>
        ...
        <p className="helper">В обычном уроке — до {{gentle:8,normal:12,more:16}[settings.vocabPace]||12} новых слов и выражений. Между ними — практика и повторы по сроку. Можно учиться полностью без звука.</p><RewardRules/>
      </section>
    ...
    {screen==='settings'&&<><ReminderSettings/><section className="settingsCard"><h2>Как удобно заниматься</h2><SpeakingModeChoice mode={settings.speakingMode} onChange={changeSpeakingMode}/></section></>}
