...
import { PHRASES, COLLOCATIONS, GRAMMAR_TASKS, LISTENING_LESSONS } from './catalog.js';
...
import { GUIDES, examplesFor, guideFor, wordHints } from './lesson-notes.js';
...
export default function App() {
  ...
  function openCheck(quarter=0) {
    const tasks=quarter?buildCheckpoint(items,progress,quarter,COLLOCATIONS,LISTENING_LESSONS,settings.level):buildFinalCheck(items,progress,COLLOCATIONS,LISTENING_LESSONS,GRAMMAR_TASKS,settings.level);
...
function CheckScreen({value,onAnswer,onNext,onDone}){if(value.done)return <section className="finishCard"><h1>{value.score}/{value.tasks.length}</h1><p>{value.passed?'Проверка пройдена. Результат сохранён.':'Пока недостаточно верных ответов. Повтори материал и попробуй снова.'}</p><button className="primary big" onClick={onDone}>К прогрессу</button></section>;const task=value.tasks[value.index];return <section className="checkpointCard" key={value.index}><div className="eyebrow">ВОПРОС {value.index+1} ИЗ {value.tasks.length}</div>{task.lesson&&<a className="audioLink" href={task.lesson.url} target="_blank" rel="noreferrer">Послушать запись ↗</a>}<h1>{task.prompt}</h1>{task.type==='recall'?<TextAnswer disabled={value.selected!==undefined} onSubmit={onAnswer}/>:<div className="options">{task.options.map(option=><button key={option} disabled={value.selected!==undefined} onClick={()=>onAnswer(option)}>{option}</button>)}</div>}{value.selected!==undefined&&<button className="primary big" onClick={onNext}>Дальше</button>}</section>;}
