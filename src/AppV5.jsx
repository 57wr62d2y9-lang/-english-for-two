...
import { LEVELS, addGoal, awardMilestone, awardLevelCompletion, ascentProgress, balanceOf, courseProgress, dayKey, finalLevelReady, isDue, levelCompletionId, redeem, reviewItem, studySlot } from './learning.js';
...
import { RewardRules, AttendanceCard, CheckRewardsSection } from './RewardOverview.jsx';
  ...
  return <div className="app v5App">
    ...
    {screen==='home'&&<>
      ...
      <section className="todayCard"><div className="todayTop"><div><div className="eyebrow">{slot==='morning'?'УТРЕННИЙ УРОК':'ВЕЧЕРНИЙ УРОК'}</div><h1>Новый шаг сегодня</h1><p>{minutes} из {settings.dailyGoal} минут за день</p></div><div className="dayRing" style={{'--done':`${Math.min(100,minutes/settings.dailyGoal*100)}%`}}><span>{Math.min(100,Math.round(minutes/settings.dailyGoal*100))}%</span></div></div>
        ...
        <div className="slotRow">{['morning','evening'].map(part=>{const amount=wallet.earned?.[`routine:${dayKey()}:${part}`]?.amount;return <span key={part} className={amount?'done':''}>{part==='morning'?'☀ Утро':'☾ Вечер'} {amount?`+$${amount}`:'$1 за урок'}</span>;})}</div>
